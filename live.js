...
async function applyExtra(){
  try {
    ...
    if (j.updated) {                                     // 서버 수집 시각(UTC ISO) → KST 'MM.DD HH:MM' (화면엔 '서버수집' 으로 표기)
      const d = new Date(j.updated.replace(/(T\d{2}:\d{2})Z$/, '$1:00Z'));
      if (!isNaN(d)) {
        const k = new Date(d.getTime() + 9 * 3600 * 1000), p = n => String(n).padStart(2, '0');
        window.__extraAt = `${p(k.getUTCMonth() + 1)}.${p(k.getUTCDate())} ${p(k.getUTCHours())}:${p(k.getUTCMinutes())}`;
      }
    }
    let n = 0;
