...
function renderGauge(score){
  ...
  host.innerHTML = `
    ...
    <div class="lg-zone" style="color:${z.color}">${zoneLab(z)}</div>
    <div class="lg-track">
...
function bindTooltip(el, it, catColor){
  el.addEventListener('mouseenter', () => {
    ...
    tooltipEl.innerHTML =
      ...
      ((v => v ? `<div class="tt-val">${esc(v)}</div>` : '')(valOf(it).trim())) +
...
function renderDataStat(){
  ...
  const brw = window.__liveAt ? `⟳ ${window.__liveAt} ${L('updated')}` : '';
  const srv = window.__extraAt ? `${L('srv')} ${window.__extraAt}` : '';   // 서버 수집(live-extra.json) 실제 시각 — 브라우저 '지금' 시각과 구분
  const stamp = [brw, srv].filter(Boolean).join(' · ') || (CONFIG.updated ? `${L('asof')} ${CONFIG.updated}` : '');
  host.innerHTML =
...
function renderTrend(){
  ...
  const h = (typeof HISTORY !== 'undefined' ? HISTORY : []).slice(-30);   // 최근 30일만
  if (h.length < 2) {
    host.innerHTML = `<div class="trend-empty">${L('trendEmpty')}</div>`;
    return;
  }
  h[h.length - 1] = { d: h[h.length - 1].d, s: totalScore() };   // 끝점 = 현재 총점
  const VW = 1000, H = 96, padT = 14, padB = 16, padX = 8;
  const vals = h.map(p => p.s);
  let lo = Math.min(500, ...vals), hi = Math.max(500, ...vals);
  const pad = Math.max(18, (hi - lo) * 0.2); lo = Math.max(0, lo - pad); hi = Math.min(1000, hi + pad);
  if (hi === lo) hi = lo + 1;
  ...
  const area = `M ${X(0).toFixed(1)},${H - padB} L ${pts.join(' L ')} L ${X(h.length - 1).toFixed(1)},${H - padB} Z`;
  const y500 = Y(500);

  // 라벨 지점: 양끝 + 중간 균등 = 최대 7개(데이터 적으면 그만큼). SVG는 왜곡되니 HTML 오버레이(%)로 얹음.
  const N = Math.min(7, h.length);
  const marks = [...new Set(Array.from({ length: N }, (_, k) => Math.round(k * (h.length - 1) / (N - 1))))]
    .map(i => ({
      xp: X(i) / VW * 100, yp: Y(h[i].s) / H * 100, s: h[i].s, d: h[i].d,
      al: i === 0 ? 'left' : i === h.length - 1 ? 'right' : 'center', last: i === h.length - 1,
    }));
  const val = marks.map(m => `<div class="tp-val tp-${m.al}" style="left:${m.xp.toFixed(2)}%;top:${m.yp.toFixed(2)}%">${m.s}</div>`).join('');
  const dots = marks.map(m => `<div class="tp-dot${m.last ? ' tp-dot-last' : ''}" style="left:${m.xp.toFixed(2)}%;top:${m.yp.toFixed(2)}%;background:${zc}"></div>`).join('');
  const dates = marks.map(m => `<div class="tp-date tp-${m.al}" style="left:${m.xp.toFixed(2)}%">${m.d}</div>`).join('');
  ...
  host.innerHTML =
    ...
    +     `<line x1="${padX}" y1="${y500.toFixed(1)}" x2="${VW - padX}" y2="${y500.toFixed(1)}" stroke="rgba(214,224,239,.16)" stroke-width="1" stroke-dasharray="5 5"/>`
    ...
    +   `</svg>`
    +   `<span class="tp-500" style="top:${(y500 / H * 100).toFixed(2)}%">500</span>`
    +   val + dots
    + `</div>`
    + `<div class="trend-dates">${dates}</div>`;
}
function render(){ const t = totalScore(); renderGauge(t); renderTrend(); renderCatBreak(); renderHeatmap(); renderDataStat(); }
